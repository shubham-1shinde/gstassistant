  ...
  return (
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-semibold text-gray-800">Select Business</h1>
          <p className="text-gray-500 text-sm mt-1">Choose a business to continue</p>
        </div>
        <button
          onClick={() => setShowModal(true)}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2.5 rounded-lg text-sm font-semibold shadow-md transition-all hover:shadow-lg hover:-translate-y-0.5"
        >
          <Plus size={18} />
          Create New Business
        </button>
      ...
      {businesses.length === 0 ? (
        <div className="bg-white rounded-xl border p-16 text-center text-gray-400">
          ...
          <p className="text-sm">No businesses yet. Add your first business to get started.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {businesses.map((biz) => (
            <button
              key={biz.id}
              className={`text-left bg-white rounded-xl border-2 p-6 shadow-sm transition-all duration-200 hover:shadow-md hover:-translate-y-0.5 group ${
                selected === biz.id
                  ? "border-blue-500 ring-2 ring-blue-100"
                  : "border-gray-100 hover:border-blue-300"
              }`}
              onClick={async () => {
                const res = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/v1/business/get-business`, { _id: biz._id });
                //console.log("Selected card response", biz._id);
                const businessData = res.data.data;
                if (businessData) {
                  dispatch(currentBusiness({businessData}));
                  navigate("/");
            ...
            >
              ...
              {/* Business Name */}
              <h2 className="text-base font-bold text-gray-800 mb-1 group-hover:text-blue-600 transition-colors">
                {biz.businessName}
              </h2>
              ...
              <div className={`mt-4 text-xs font-semibold transition-colors ${
                selected === biz.id ? "text-blue-600" : "text-gray-300 group-hover:text-blue-400"
              }`}>
